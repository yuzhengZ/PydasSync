...
"""
mSync: recursively synchronize data between a local directory and a Midas folder.
"""
...
def sanity_check(sync_setting):
    ...
    if not os.path.isdir(sync_setting.local_root_dir):
        ...
        return False
    elif sync_setting.mode == 'download' and not os.access(sync_setting.local_root_dir, os.W_OK):
        ...
        for filename in files:
            ...
            if root in sync_status.only_local['entire_dirs']:
                ...
                if midas_item_info['revisions'] \
                   ...
                   and local_file_checksum  == midas_item_info['revisions'][-1]['bitstreams'][0]['checksum']:
                        continue
...
def download_data_to_local(sync_setting):
    ...
    pydas.download(pydas_root_download_source, local_path=local_root_dir)
...
def synchronize_data(sync_setting):
    ...
    """
    sync_status = check_sync_status(sync_setting)
...
def main():
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hm:l:u:e:a:f:", ["help", "mode=", "localdir=", "url=", "email=", "apikey=", "folderid=" ])
    ...
    for opt, arg in opts:
        if opt in ('-h', '--help'):
            print 'mSync.py -m (check|upload|download) -l <local_root_directory> -u <midas_url> -e <midas_user_email>  -a <midas_api_key> -f <midas_root_folder_id>'
            sys.exit()
        elif opt in ('-m', '--mode'):
            mode = arg.lower()
